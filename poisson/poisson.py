...
def main(lr, train_path, eval_path, save_path):
    ...
    plt.scatter(_, y_pred, alpha=0.5)
...
class PoissonRegression:
    ...
    def fit(self, x, y):
        ...
        for i in range(self.max_iter):
            # Calculate eta and gradient
            eta = np.dot(x, self.theta)
            gradient = np.dot(x.T, (y - np.exp(eta)))
            ...
            # Print loss value during training if verbose is True
            if self.verbose and i % 1000 == 0:
                loss = np.sum(y * eta - np.exp(eta))
